...
const themesDir = resolve(import.meta.dirname ?? new URL('.', import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1'))

const themes = readdirSync(themesDir, { withFileTypes: true })
    .filter(d => d.isDirectory() && existsSync(join(themesDir, d.name, 'watch.mjs')))
    .map(d => d.name)

const COLORS = ['\x1b[36m', '\x1b[33m', '\x1b[35m', '\x1b[32m', '\x1b[34m', '\x1b[31m', '\x1b[37m', '\x1b[96m', '\x1b[93m']
const RESET = '\x1b[0m'

const pad = Math.max(...themes.map(s => s.length))

console.log(`[watch-all] Starting watchers for: ${themes.join(', ')}\n`)

themes.forEach((name, i) => {
    const color = COLORS[i % COLORS.length]
    const prefix = `${color}[${name.padEnd(pad)}]${RESET} `
    const cwd = join(themesDir, name)
