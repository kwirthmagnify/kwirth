...
export const TerminalInstance: React.FC<ITerminalInstanceProps> = (props: ITerminalInstanceProps) => {
    ...
    useEffect(() => {
            ...
            if (containerRef.current.offsetWidth > 0 && containerRef.current.offsetHeight > 0) {
                ...
                rafRef.current = requestAnimationFrame(() => {
                    if (!isDisposed && term && (term as any)._core) {
                        try { fitAddon.fit() } catch (e) { console.warn('Error fitting terminal:', e) }
                    }
                })
            }
        })
        ...
        const timeoutFocus = setTimeout(() => { if (!isDisposed) term.focus() }, 200)
    ...
    return <div ref={containerRef} style={{ width: '100%', height: '100%', flex: 1, minHeight: 0, overflow: 'hidden', background: 'black', paddingBottom: '16px' }} />
}
