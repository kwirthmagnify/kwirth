...
const OpsTabContent: React.FC<IContentProps> = (props: IContentProps) => {
    ...
    enum LaunchActionEnum { INFO, TERMINAL, RESTART }
    ...
    const onAsyncData = (data: any) => {
        if (data.event === 'describe') {
            const content = <Box sx={{ width: '600px', height: '400px', overflow: 'auto' }}><JsonView data={data.data} style={defaultStyles} /></Box>
    ...
    useEffect(() => {
        ...
        if (opsData.selectedTerminal) setSelectedTerminal(opsData.selectedTerminal)
        if (!opsData.terminalManager.onClose) opsData.terminalManager.onClose = closeTerminal
        return () => { window.removeEventListener('keydown', onKeyDown); setSelectedTerminal(undefined) }
    }, [])
    ...
    const onKeyDown = (event: any) => {
        ...
        if (key.startsWith('F') && key.length > 1) {
            switch (key) {
                case 'F12': setSelectedTerminal(undefined); opsData.selectedTerminal = undefined; break
                case 'F11':
                    if (opsData.terminalManager.terminals.size === 0) setMsgBox(MsgBoxOk('Terminal', 'You have no terminal consoles open.', setMsgBox))
                    else { setShowSelector(true); event.stopPropagation(); event.preventDefault() }
                    break
                default:
                    let knum = +key.substring(1)
                    let manterm = Array.from(opsData.terminalManager.terminals.entries()).find(tm => tm[1].index === knum)
                    if (manterm) { setSelectedTerminal(manterm[0]); opsData.selectedTerminal = manterm[0] }
                    break
            }
    ...
    const onSelectNewTerm = (id?: string) => {
        ...
        if (id) { opsData.selectedTerminal = id; setSelectedTerminal(id) }
    }

    const assignIndex = (_key: string) => {
        for (let i = 1; i < 11; i++) {
            if (!Array.from(opsData.terminalManager.terminals.values()).some(t => t.index === i)) return i
        }
        return 0
    }
        ...
        switch (type) {
            case LaunchActionEnum.TERMINAL: {
                let instanceConfig: IInstanceConfig = {
                    ...
                    accessKey: props.channelObject.accessString!, instance: props.channelObject.instanceId,
                    namespace: so.namespace, group: '', pod: so.pod, container: so.container,
                    objects: EInstanceConfigObject.PODS, scope: '', view: EInstanceConfigView.NONE,
            ...
            case LaunchActionEnum.RESTART: {
                let opsMessage: IOpsMessage = {
                    flow: EInstanceMessageFlow.REQUEST, action: EInstanceMessageAction.COMMAND,
                    channel: EInstanceMessageChannel.OPS, type: EInstanceMessageType.DATA,
                    accessKey: props.channelObject.accessString!, instance: props.channelObject.instanceId,
                    id: uuid(), command: EOpsCommand.RESTART,
                    namespace: so.namespace, group: '', pod: so.pod, container: so.container, params: [], msgtype: 'opsmessage'
                }
        ...
        switch (opt) {
            case EMenuObjectOption.DESCRIBE: {
                let msg: IOpsMessage = {
                    ...
                    channel: EInstanceMessageChannel.OPS, type: EInstanceMessageType.DATA,
                    accessKey: props.channelObject.accessString!, instance: props.channelObject.instanceId,
                    id: uuid(), command: EOpsCommand.DESCRIBE,
                    namespace: so.namespace, group: '', pod: so.pod, container: so.container, params: [], msgtype: 'opsmessage'
                }
            ...
            case EMenuObjectOption.RESTARTPOD:
                setMsgBox(MsgBoxYesNo('Restart pod', `Are you sure you want to restart pod '${so.pod}' in '${so.namespace}' namespace?`, setMsgBox, (button) => {
                    if (button === MsgBoxButtons.Yes) {
                        let msg: IOpsMessage = { msgtype: 'opsmessage', action: EInstanceMessageAction.COMMAND, flow: EInstanceMessageFlow.REQUEST, type: EInstanceMessageType.DATA, channel: EInstanceMessageChannel.OPS, instance: props.channelObject.instanceId, id: '1', accessKey: props.channelObject.accessString!, command: EOpsCommand.RESTARTPOD, namespace: so.namespace, group: '', pod: so.pod, container: '' }
                        props.channelObject.webSocket?.send(JSON.stringify(msg))
                    }
                }))
                break
            case EMenuObjectOption.RESTARTNS:
                setMsgBox(MsgBoxYesNo('Restart namespace', `Are you sure you want to restart namespace '${so.namespace}'?`, setMsgBox, (button) => {
                    if (button === MsgBoxButtons.Yes) {
                        let msg: IOpsMessage = { msgtype: 'opsmessage', action: EInstanceMessageAction.COMMAND, flow: EInstanceMessageFlow.IMMEDIATE, type: EInstanceMessageType.DATA, channel: EInstanceMessageChannel.OPS, instance: '', id: '1', accessKey: props.channelObject.accessString!, command: EOpsCommand.RESTARTNS, namespace: so.namespace, group: '', pod: '', container: '' }
                        props.channelObject.webSocket?.send(JSON.stringify(msg))
                    }
                }))
                break
            case EMenuObjectOption.VIEWMETRICS: {
                let metricsResource: IResourceSelected = { channelId: 'metrics', clusterName: props.channelObject.clusterName, view: props.channelObject.view, namespaces: [so.namespace], controllers: [], pods: [so.pod], containers: [so.container], name: `${so.namespace}-${so.pod}+${so.container}` }
                let metricsConfig: IMetricsConfig = { depth: 50, width: 3, lineHeight: 300, configurable: true, compact: false, legend: true, merge: false, stack: false, chart: EChartType.LineChart, metricsDefault: {} }
                let metricsInstanceConfig: IMetricsInstanceConfig = { mode: EMetricsConfigMode.STREAM, aggregate: false, interval: 15, metrics: ['kwirth_container_cpu_percentage', 'kwirth_container_memory_percentage', 'kwirth_container_transmit_mbps', 'kwirth_container_receive_mbps', 'kwirth_container_write_mbps', 'kwirth_container_read_mbps'] }
                props.channelObject.createTab?.(metricsResource, true, { config: metricsConfig, instanceConfig: metricsInstanceConfig })
                break
            }
            case EMenuObjectOption.VIEWLOG: {
                let logResource: IResourceSelected = { channelId: 'log', clusterName: props.channelObject.clusterName, view: props.channelObject.view, namespaces: [so.namespace], controllers: [], pods: [so.pod], containers: [so.container], name: `${so.namespace}-${so.pod}+${so.container}` }
                let logConfig: ILogConfig = { fromNowOn: false, startDiagnostics: false, follow: true, showNames: false, maxMessages: 5000, maxPerPodMessages: 5000, sortOrder: ELogSortOrder.TIME }
                let logInstanceConfig: ILogInstanceConfig = { previous: false, timestamp: false, fromStart: false }
                props.channelObject.createTab?.(logResource, true, { config: logConfig, instanceConfig: logInstanceConfig })
                break
            }
        }
    }
    ...
    if (opsConfig.launchShell) return (
    ...
    return (
        <Box ref={opsBoxRef} sx={{ display: 'flex', flexDirection: 'column', overflowY: 'auto', overflowX: 'hidden', flexGrow: 1, height: `${opsBoxHeight}px`, p: 2, gap: 2 }}>
            {showSelector && <SelectTerminal onSelect={onSelectNewTerm} current={selectedTerminal!} opsData={opsData} />}
            {opsData.started && !selectedTerminal && (
                <Stack direction='row' spacing={2} alignItems='stretch' sx={{ flexGrow: 1, minHeight: 0 }}>
                            ...
                            {opsData.scopedObjects.filter(so => so.namespace.includes(filter) || so.pod.includes(filter) || so.container.includes(filter)).map((so, index) => (
                                <ListItem key={index}>
                                    <Stack direction='row' sx={{ width: '100%' }} alignItems='center'>
                                        <Typography>{so.namespace + '/' + so.pod + '/' + so.container}</Typography>
                                        <Typography flex={1} />
                                        <Stack direction='row' alignItems='center'>
                                            <Tooltip title='Restart container'><IconButton onClick={() => launch(LaunchActionEnum.RESTART, so)}><RestartAlt /></IconButton></Tooltip>
                                            <IconButton onClick={() => launch(LaunchActionEnum.TERMINAL, so)} disabled={opsData.terminalManager.terminals.has(so.namespace + '/' + so.pod + '/' + so.container)}><Terminal /></IconButton>
                                            <IconButton onClick={(event) => { setAnchorMenuChart(event.currentTarget); setSelectedScopedObject(so) }}><MoreVert /></IconButton>
                                        </Stack>
                                    </Stack>
                                </ListItem>
                            ))}
                    ...
                    <Card sx={{ width: '40%', display: 'flex', flexDirection: 'column', maxHeight: '100%' }}>
                        <CardHeader title={<>XTerm</>} />
                        <CardContent sx={{ overflowY: 'auto' }}>
                            {opsData.terminalManager.terminals.size === 0 && <>You have no open terminals.</>}
                            {opsData.terminalManager.terminals.size > 0 && Array.from(opsData.terminalManager.terminals.keys()).map((key, index) => (
                                <ListItem key={index}>
                                    <Stack direction='row' sx={{ width: '100%' }} alignItems='center'>
                                        <ListItemButton onClick={() => onSelectNewTerm(key)}>
                                            <Typography flex={1}>{key}</Typography>
                                            {opsData.terminalManager.terminals.get(key)!.index > 0 ? <Typography width='32px'>{'F' + opsData.terminalManager.terminals.get(key)!.index}</Typography> : <Typography width='32px' />}
                                        </ListItemButton>
                                        <IconButton onClick={() => deleteSession(key)}><Delete /></IconButton>
                                    </Stack>
                                </ListItem>
                            ))}
                        </CardContent>
