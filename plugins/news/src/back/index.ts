...
class NewsChannel {
    ...
    getChannelScopeLevel = (_scope: string): number => 0
    ...
    addObject = async (webSocket: WebSocket, instanceConfig: IInstanceConfig, podNamespace: string, podName: string, containerName: string): Promise<boolean> => {
        ...
        if (!instance) {
            ...
            instance.pollInterval = setInterval((ws: WebSocket, i: IInstance) => this.pollFeeds(ws, i), POLL_INTERVAL_MS, webSocket, instance) as unknown as ReturnType<typeof setInterval>
        ...
        return new Promise((resolve, reject) => {
            ...
            const req = protocol.get(url, { headers: { 'User-Agent': 'kwirth-news/1.0' } }, (res) => {
                if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                    resolve(this.fetchUrl(res.headers.location)); return
                }
                let data = ''
                ...
                for (const item of items) {
                    if (!instance.seenLinks.has(item.link)) {
                        ...
                        const msg: INewsMessageResponse = {
                            msgtype: 'newsmessageresponse', channel: 'news',
                            action: EInstanceMessageAction.NONE, flow: EInstanceMessageFlow.UNSOLICITED,
                            type: EInstanceMessageType.DATA, instance: instance.instanceId, item
                        }
    ...
    private sendSignalMessage = (ws: WebSocket, action: EInstanceMessageAction, flow: EInstanceMessageFlow, level: ESignalMessageLevel, instanceId: string, text: string): void => {
        const resp: ISignalMessage = { action, flow, channel: 'news', instance: instanceId, type: EInstanceMessageType.SIGNAL, text, level }
