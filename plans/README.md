...
| [provider-handle](provider-handle/PLAN.md) | lo que el core entrega a un consumidor deja de ser el `IProvider` real: un handle por (canal, instancia) | el handle, sin empezar. De su backlog ya se arregló lo que dejaba el grafo inservible (el registro borraba la arista con la primera baja) y el aviso de Status; sigue abierto que `started` significa "router montado" y no "arrancado" |
