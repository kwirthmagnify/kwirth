# Providers
As of Kwirth version 0.5.40, we have converted data sources (the ones we use for extracting data fromo Kubernetes) into a 'provider'. ths means:

  - Kwirth core can be extended adding new providers.
  - Providers are now stabdadized, adn can be moved away form Kwirrh core (converting them into plugins, for example).

In the first version fo th eprovider subsystem there exist three providers:

  - **[Tick](./providers?id=tick)**. Alerts based on los messages. Log messages are processed at Kwirth core, so you only receive alerts accordingn to your channel config.
...
### Features
Tick provider has no condfiguration, the interval for the Tick is fixed. The source for ticks (a setInterval in fact) is **unique** for all subscribers, that is, one only source for ticks *for each cluster*.

!> The source is linked to a running instanc, not to Kwirth core, so you will have a unique Tick provider for each cluster.
...
### What for
It is intendeed to be used by the channels requiring information on what's taking place inside Kubernetes, since this providers captures **all** the activity of the cluster.

### Features
Main features of Events provider are:

  - Capture all ADDED/MODIFIED/DELETED events.
  - Subcribers can set the list of objects they want to subscrbe to. For example, `pinocchio` channel is just subscribed to 'Pod' kind events, while `magnify` channel is subscribed to all the kinds the user has seleceted inf `magnify` front.
...
### What for
You can obtain information obout object **before** they are ADDED/DELETED/MODIFIED. In near futur channels will be able to answer `validating` channel regarding the review process, stating if the review is accepted or denied.

### Features
Provider sends validating request to all sbuscirbed channels according to the initial configuration:

### Use
Subscribe to provither the usual way in the moment your channel needs it, **but not in the constructor**. The very first moment you can subscribe is the `startChannel` function. You typically subscribe to `validating` provider like this:
