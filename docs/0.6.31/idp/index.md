...
- **Authorization** is still done by kwirth. The person must already exist as a user in kwirth (see [User management](/0.6.31/usermanagement)) and be bound to that specific IdP, otherwise access is denied.
...
2. A kwirth user exists whose **id is that email** (created from [User management](/0.6.31/usermanagement)).
...
- [Google / Gmail](/0.6.31/idp/google) — OIDC
- [GitLab](/0.6.31/idp/gitlab) — OIDC (cloud + self-managed)
- [GitHub](/0.6.31/idp/github) — OAuth2 (cloud + enterprise server)
...
[IdP security notes](/0.6.31/idp/security).
