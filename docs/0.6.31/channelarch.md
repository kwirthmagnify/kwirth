...
Channels have been fully integrated into the **plugin system**, exactly the way [Backstage plugins](https://backstage.io/plugins) work. Each channel is now a self-contained plugin that can be installed, updated, or removed at runtime without restarting kwirth.
...
## Existing channels
Channel subsystem started in kwirth 0.3.160. Nowadays, all channels are delivered as **plugins** — two of them (Metrics and Magnify) remain bundled inside kwirth core, while the rest are independent plugin packages that can be installed on demand.
