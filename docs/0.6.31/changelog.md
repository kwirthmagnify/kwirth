...
### Five new extension types

Kwirth went from six extension families to **eleven**. Everything below installs, updates and uninstalls at runtime from its own manager, exactly like plugins and providers already did.

  - **Identity providers (IdP)**: SSO is now an extension. Installable connectors for **Google** (OIDC), **GitLab** (cloud and self-managed, OIDC) and **GitHub** (cloud and Enterprise Server, OAuth2), each with its own configuration form. The login page shows one button per configured provider — or a dropdown when there are several — and OIDC claim mapping lets you say which claim carries the user name, the email and the groups, with multi-tenant issuer validation for providers that serve more than one organisation.

  - **Login extensions**: the login page itself is pluggable. A login extension is a branded page — background, colours, copy — that can also **enforce which channels** its users may open, so the same kwirth can present one door to the operations team and a different one, with a reduced channel set, to a business audience. Administrators are never locked out: the admin bypass is always available.

  - **Webhooks**: the inbound counterpart of senders. Where a sender pushes messages out of kwirth, a webhook lets an external system push events **in** over HTTP. Each webhook extension registers its endpoints and tokens, holds several named configurations, and delivers to subscribers under a **strict (webhook, configuration) pair** — so two teams can feed the same webhook through different configurations without seeing each other's traffic. Webhooks can be bundled in packs like any other extension.

  - **Documentation packages**: self-contained docsify sites that kwirth serves itself, with no external host and no internet access required. This guide is one of them. Every core dialog now carries a **help button** that opens the relevant page of the right manual, so documentation is reachable from where the question appears rather than from a bookmark.

  - **AI toolsets**: packaged groups of **tools an LLM can call** — list the pods, describe the failing one, read its logs, check a metric. They are installed like any other extension and then **granted** to the plugins that may use them, which is what makes the capability of an AI channel an administrative decision instead of a property of the code. See [AI toolsets](guide/extensions/aitoolsets/index).

### One manager for every extension family

All eleven families are now managed by **the same dialog**. It is not a coincidence of style: there is a single generic manager driven by a descriptor per family, so a change to how extensions are listed, filtered, installed or configured happens once and reaches all of them.

What it brought along the way:

  - **Origin and state, read at a glance.** Each card separates *where this copy came from* (public catalog, a private marketplace — named — or a local `dev` build) from *what it has now* (how many configurations it holds, whether a pack owns it).
  - **Four ways to configure, one place to open them.** An extension declares whether it wants a typed form, a list of named configurations (with a shared base configuration and export/import), a free JSON editor, or a UI it provides itself. **The gear is disabled when there is nothing to configure**, with a tooltip saying so, instead of opening an empty dialog.
  - **Requirements between extensions.** An extension can declare what it `requires` and what it merely `uses`; the manager resolves that against what is installed and says so on the card, before you wonder why something does nothing.
  - **Restart awareness.** Extensions that own an HTTP route or open a listener declare `requiresRestart`, and the manager warns you when installing, updating **and uninstalling** them.

### Private marketplaces

The extension catalog is no longer a single public list. Kwirth can now read **several marketplaces**, including private ones behind authentication, with the manifest read authenticated **separately** from the package download — which is what a private registry in front of a public CDN actually needs.

  - Supported manifest hosts: **GitLab**, **GitHub** and **Azure DevOps**, each with its own authentication scheme.
  - **Provenance** travels with the extension: every manager shows which marketplace a given copy came from, so an installation assembled from three sources can still be audited.
  - Marketplace credentials are stored as secrets and shown masked with a reveal toggle.

### Security and access control

  - **RBAC declared by the plugin.** A plugin publishes its own scope catalog at runtime, and the core exposes it; the front no longer carries a hardcoded list of scopes. Adding a channel with its own permissions no longer means touching the core.
  - **Admin scope enforced** on the sensitive routes — users, API keys and identity providers — and channel start gated by the channel list of the API key that asks.
  - **Passwords are hashed** end to end: SHA-256 in the browser so the plain password never travels, bcrypt on the server for storage.
  - **Secrets encrypted at rest** with AES-256-GCM when kwirth uses filesystem storage, and a `KWIRTH_STORE` environment variable to place that store where you want it.

### Multi-cluster federation

A channel can now work **across clusters**: a tab opened on one kwirth can reach the channels of another, back to back over WebSocket, reusing the credentials stored for that cluster.

  - **Keepalive** ping/pong on federation links, which both detects silent drops and stops load balancers from closing an idle connection.
  - **Self-healing**: a link that comes back re-starts the remote channel on its own, watched by an instance watchdog, so a reconnection does not leave a live-looking tab receiving nothing.
  - The cluster list each channel receives is **stamped with what it can actually do there** — whether the cluster is reachable and which scopes the stored key holds — so a channel can disable what would fail instead of failing.

### AI, beyond a single channel

  - **AI configuration moved to the core.** Providers and models are configured once, in one dialog, and shared by every AI-capable extension — with a *Load models* action that asks the provider what it offers rather than making you type model ids.
  - More providers: **Anthropic** and any **OpenAI-compatible** endpoint, on top of the existing ones.
  - **Structured vision**: a multimodal helper that returns typed output from an image, not free text.
  - **An agent engine** with tool effects: tools declare whether they read or write, and a read-only agent is denied writes at invocation — the decision is about what a tool *does*, not which package it came from.

### Data and storage

  - **`common-sql`**: a shared relational storage service (knex/PostgreSQL) exposed to extensions by the core, with per-consumer pool sizing and a warning when the connection budget is exceeded. Extensions that need real persistence no longer each invent it.
  - **New providers**: `business` (installable, for business data), `http-pull-push` (polls HTTP endpoints and pushes the result to subscribers), and a **Trivy** provider that watches the vulnerability and RBAC assessment CRDs and dispatches them as events.
  - **`provider-debug`**: a channel that shows, raw, whatever a provider is emitting — the quickest way to find out whether a problem is in the provider or in the consumer.
  - The **events provider** now pushes Kubernetes Events as well, so plugins subscribe to one source instead of creating their own informers.

### Themes and homepages, in two tiers

  - **Personal choice**: every user picks their theme and homepage in *User settings*.
  - **Administrative assignment**: an administrator can bind a theme to a *plugin*, so a channel always renders with the look it was designed for, whatever the user's personal preference elsewhere.
  - Homepages show their setup dialog when selected, and the activate/deactivate buttons disappeared from both managers — choosing one *is* activating it.

### Settings, portability and placement

  - **Kwirth settings are persisted** and can be **exported and imported**, so an installation can be reproduced elsewhere instead of reassembled by hand.
  - **Channel placement**: a channel declares how many instances it admits and whether it runs local or remote, and a channel that does not need a cluster at all can declare the `none` view.
  - **Plugin installation config**: plugins can hold their own install-time configuration, editable from the manager.
  - **Update notifications** for every extension type, in every environment.
