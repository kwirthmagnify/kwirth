...
interface MenuNotificationProps {
    ...
    onClose: () => void
    onRefresh: () => void
    renderIcon?: (channelId: string | undefined) => React.ReactNode   // icono por notificación (opcional; fallback = campana)
}

const MenuNotification: React.FC<MenuNotificationProps> = ({ anchorParent, onClose, notifications, onRefresh, renderIcon }) => {
                        ...
                        >
                            <Stack direction={'row'} alignItems={'start'}>
                                <span style={{ marginTop: 2 }}>
                                    {renderIcon ? renderIcon(msg.channelId) : <NotificationsIcon sx={{ fontSize: 20, opacity: 0.5 }} />}
                                </span>
                                <ListItemText
                                    primary={msg.text}
