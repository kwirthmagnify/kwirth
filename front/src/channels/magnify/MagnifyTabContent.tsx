...
import { CloudQueue, Edit, ExitToApp, Notifications, NotificationsActive, Search } from '@kwirthmagnify/kwirth-common-front/icons'
import { CloudOff, EditOff, List } from './icons'
...
const MagnifyTabContent: React.FC<IContentProps> = (props:IContentProps) => {
        ...
        if (existingWin) {
            ...
            let win:IContentWindow = {
                ...
                data: {
                    ...
                    containerSelectionOptions: new Map([['log',2],['metrics',2],['fileman',2],['trivy',0],['topology',0],['ops',1],['evict',0],['cordon',0],['uncordon',0],['drain',0], ['scale',0], ['restart',0], ['trigger',0], ['suspend',0], ['resume',0]  ])
    ...
    const launchObjectExternal = (channel:string, files:IFileObject[], view: EInstanceConfigView, data:any, container: string|undefined ) => {
        ...
        if (channel==='topology') {width = window.innerWidth * 0.9; height = window.innerHeight * 0.9}
    ...
    const setPodActions = () => {
        ...
        setLeftItem(spcPod,'trivy', (p:string[], currentTarget:Element) => podGroupAction('trivy', p, currentTarget), () => props.channelObject.frontChannels?.has('trivy') ?? false)
