...
const IdpManagerDialog: React.FC<IIdpManagerDialogProps> = (props: IIdpManagerDialogProps) => {
    ...
    const resolveSource = (from?: string): React.ReactElement | null => {
        ...
        if (from === 'bundled') return <Chip label='bundled' size='small' variant='outlined' color='secondary' sx={compactChip} />
        if (from === 'local') return <Chip icon={<FolderOpen />} label='Local file' size='small' variant='outlined' sx={compactChip} />
                                        ...
                                        <Stack direction='row' alignItems='flex-start' spacing={1.5}>
                                            ...
                                            <Box flex={1} minWidth={0}>
                                                <Stack direction='row' alignItems='center' spacing={0.5} sx={{ width: '100%' }}>
                                                    <Typography variant='body2' fontWeight='bold' sx={extensionCardTitleSx}>{c.label}</Typography>
