...
const ProviderManagerDialog: React.FC<IProviderManagerDialogProps> = (props: IProviderManagerDialogProps) => {
    ...
    const loadInstalled = async () => {
        try {
            ...
            // El endpoint devuelve tambien los providers de core (events, metrics), que no son
            // extensiones: no se instalan ni se desinstalan, asi que no pintan nada en el gestor.
            setInstalled(data.filter(p => !(p as IInstalledProvider & { core?: boolean }).core))
        } catch (err) {
    ...
    const fetchManifest = async (refresh = false) => {
        ...
        try {
            const res = await fetch(`${backendUrl}/core/marketplace/${EExtensionType.PROVIDER}${refresh ? '?refresh=true' : ''}`, addGetAuthorization(accessString))
            if (!res.ok) throw new Error(`HTTP ${res.status}`)
                                        ...
                                        <Stack direction='row' alignItems='flex-start' spacing={1.5}>
                                            ...
                                            <Box flex={1} minWidth={0}>
                                                <Stack direction='row' alignItems='center' spacing={0.5} sx={{ width: '100%' }}>
                                                    <Typography variant='body2' fontWeight='bold' sx={extensionCardTitleSx}>{provider.displayName || provider.name || provider.id}</Typography>
                                ...
                                {filteredIds.map(id => {
                                    ...
                                    <Box key={id} sx={{ ...extensionCardSx, background: providerGradient(provider.name) }}>
                                        <Stack direction='row' alignItems='flex-start' spacing={1.5}>
                                            ...
                                            <Box flex={1} minWidth={0}>
                                                <Stack direction='row' alignItems='center' spacing={0.5} sx={{ width: '100%' }}>
                                                    <Typography variant='body2' fontWeight='bold' sx={extensionCardTitleSx}>{provider.displayName || provider.name}</Typography>
