...
const DocsManagerDialog: React.FC<IDocsManagerDialogProps> = (props: IDocsManagerDialogProps) => {
    ...
    const fetchManifest = async (refresh = false) => {
        ...
        try {
            const res = await fetch(`${backendUrl}/core/marketplace/${EExtensionType.DOCS}${refresh ? '?refresh=true' : ''}`, addGetAuthorization(accessString))
    ...
    const resolveSource = (installedFrom?: string): React.ReactElement | null => {
        ...
        if (installedFrom.includes('github.com/kwirthmagnify'))
            return <Chip icon={<Description />} label='Kwirth' size='small' variant='outlined' color='primary' sx={compactChip} />
    ...
    return (
        <Dialog open={true} maxWidth={false} sx={{ '& .MuiDialog-paper': { width: '72vw', maxWidth: '72vw', height: '80vh' } }}>
            <DialogTitleHelp section='guide/extensions/docs/index?id=admin-guide' docsUrl={docsUrl(backendUrl, 'core', 'kwirth')}>Manage documentation</DialogTitleHelp>
            <DialogContent>
                <Stack direction='column' spacing={2} sx={{ mt: 1 }}>
                    ...
                    {available.length > 0 && <>
                        ...
                        { /* El conmutador card/lista vale para las DOS secciones: dejar el catalogo siempre
                                ...
                                {filteredKeys.flatMap((key, i, arr) => {
                                    ...
                                    return [
                                        ...
                                        <Box key={`${key}-install`} sx={{ py: 1 }}>
                                            <Tooltip title={isInstalled(entry.targetType, entry.id) ? 'Already installed' : 'Install'}>
                                                <span>
                                                    <IconButton size='small' color='primary' disabled={isInstalled(entry.targetType, entry.id) || installingId === key} onClick={() => installFromManifest(entry)}>
                                                        {installingId === key ? <CircularProgress size={16} /> : <Download fontSize='small' />}
                                                    </IconButton>
                                                </span>
                                            </Tooltip>
                                        </Box>,
                                        ...(i < arr.length - 1 ? [<Box key={`${key}-sep`} sx={{ gridColumn: '1 / -1', borderBottom: 1, borderColor: 'divider', mx: -1.5 }} />] : [])
                                    ]
                                })}
                              </Box>
                        }
