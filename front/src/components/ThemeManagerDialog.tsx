...
const ThemeManagerDialog: React.FC<IThemeManagerDialogProps> = (props: IThemeManagerDialogProps) => {
    ...
    const fetchManifest = async (refresh = false) => {
        ...
        try {
            const res = await fetch(`${backendUrl}/core/marketplace/${EExtensionType.THEME}${refresh ? '?refresh=true' : ''}`, addGetAuthorization(accessString))
    ...
    return (
        <Dialog open={true} maxWidth={false} sx={{ '& .MuiDialog-paper': { width: '72vw', maxWidth: '72vw', height: '80vh' } }}>
            <DialogTitleHelp section='guide/extensions/themes/index?id=admin-guide' docsUrl={docsUrl(backendUrl, 'core', 'kwirth')}>Manage themes</DialogTitleHelp>
            <DialogContent>
